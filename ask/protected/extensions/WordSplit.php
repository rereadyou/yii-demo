 ...
 * 修改类名称，增加一个封装函数，方便使用
 *
 * function doSplit($str, $p = ' ')
 *
 * 使用示例：
 * $pa = new WordSplit();
 * $pa->setDebug();
 * echo $pa->doSplit($teststr);
 *
 * 周泉 2011-3-23
 */
 ...
 *  3、简单操作流程： SetSource -> StartAnalysis -> GetFinallyResult
...
class WordSplit
{
    ...
    */
	...
	function doSplit($str, $p = ' ')
	{
	    ...
	    //执行分词
    	$ret = $this->SetSource( $str );
    	if ($this->debug) {
    	    if (!$ret) {
    	        echo 'Set Source func failed.<br />';
			}
		}
    	$this->differMax = $do_multi;
    	$this->unitWord = $do_unit;

    	$this->StartAnalysis( $do_fork );
	    $string = $this->GetFinallyResult($p, $do_prop);
	    
	    $stringarr = preg_split('/\s+/', $string);
	    return $stringarr;
	}
    ...
    */
    private function _sort_finally_result()
    {
        ...
        if ($this->debug) {
            print_r($this->finallyResult);
            echo ' is finallyResult.<br /><br />';
		}
        $newarr = '';
      }
